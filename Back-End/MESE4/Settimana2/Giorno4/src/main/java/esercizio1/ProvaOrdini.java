...
//esercizio 2
        Map<Cliente,Double>m2= ordini.stream().collect(Collectors.groupingBy(Ordine::getCliente,Collectors
                .summingDouble(value -> value.getProdotti().stream().mapToDouble(Prodotto::getPrezzo).sum())));
        m2.forEach((cliente, sommaOrdini)-> System.out.println(cliente +" -> "+ sommaOrdini));
//esercizio 3

        Map<String, Optional<Prodotto>>m3= prodotti.stream().collect(Collectors.groupingBy
                (Prodotto::getCategoria, Collectors.maxBy(Comparator.comparingDouble(Prodotto::getPrezzo))));
        m3.forEach((categoria, prodottoMax)-> System.out.println(categoria +" -> "+ prodottoMax.get()));
//esercizio4
       Double media2= ordini.stream().collect(Collectors.averagingDouble
               (value -> value.getProdotti().stream().mapToDouble(Prodotto::getPrezzo).sum()));
